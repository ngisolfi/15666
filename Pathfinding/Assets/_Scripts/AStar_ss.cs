...
public class AStar_ss : MonoBehaviour {
	...
	void Start () {
		...
		getGrid = gridobject.GetComponentInChildren<Make_grid_raycast> ();
	...
	void ComputePath() {
		plan.Clear ();
		int counter = 0;
		while( counter<1000 && !OPEN.Contains(new Node((int)goalGrid.x,(int)goalGrid.y,0.0f,0.0f,null))){
			//while(counter<20){
			...
			if(OPEN.Count>0){
				...
				getGrid.cMap[currState.gridx,currState.gridz]=1;
	...
	void PublishSolution() {
		if(CLOSED.Count>0){
			Node temp = CLOSED[CLOSED.Count-1];
	...
	void MK_Maps() {
		...
		for (int i=0; i<width; i++) {
			for (int j=0; j<height; j++) {
				hMap [i, j] = 	epsilon*((float)Mathf.Max (
					Mathf.Abs (i - goalGrid.x),
					Mathf.Abs (j - goalGrid.y)) +
				                        0.4f*(float)Mathf.Min (
					Mathf.Abs (i - goalGrid.x),
					Mathf.Abs (j - goalGrid.y)));
